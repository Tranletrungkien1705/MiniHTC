...
app.MapPost("/api/bankgrts/{no}/edit-deferred", async (string no, GrtDeferredEditDto dto, AppDbContext db, ITenantContext t) =>
{
    no = no.Trim().ToUpperInvariant();
    var g = await db.BankGuarantees.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.GuaranteeNo == no);
    if (g is null) return Results.NotFound(new { no });
...
app.MapGet("/api/report/storagerearrange", async (AppDbContext db, ITenantContext t, string? storageFrom, string? storageTo, string? status) =>
{
    ...
    var reqs = await q.ToListAsync();
    var ids = reqs.Select(r => r.Id).ToHashSet();
    var lq = db.StorageRearrangeDetails.Where(l => l.OrgId == t.OrgId && ids.Contains(l.StorageRearrangeId));
    ...
    var byStatus = reqs.GroupBy(r => r.Status).Select(g => new { status = g.Key, count = g.Count() }).OrderByDescending(x => x.count).ToList();
    var byDay = reqs.GroupBy(r => r.CreatedAt.ToString("yyyy-MM-dd")).Select(g => new { day = g.Key, count = g.Count() }).OrderByDescending(x => x.day).Take(30).ToList();
    var carByReq = lines.GroupBy(l => l.StorageRearrangeId).ToDictionary(g => g.Key, g => g.Count());
    var detail = reqs.Where(r => carByReq.ContainsKey(r.Id) || string.IsNullOrEmpty(storageFrom + storageTo))
        .OrderByDescending(r => r.Id).Take(500).Select(r => new
        {
            r.SCNo, r.Status, cars = carByReq.TryGetValue(r.Id, out var c) ? c : 0, createdAt = r.CreatedAt.ToString("yyyy-MM-dd")
        }).ToList();
    return Results.Ok(new { total = reqs.Count, totalCars = lines.Count, confirmed = reqs.Count(r => r.Status == "Confirmed"), byRoute, byStatus, byDay, detail });
}).RequireAuthorization();
...
app.MapGet("/api/redeeminvoicerequests/{id}", async (long id, AppDbContext db, ITenantContext t) =>
{
...
{
    var lines = dto.Lines ?? new List<RedeemInvoiceRequestLineDto>();
...
app.MapPost("/api/redeeminvoicerequests/{id}/{action}", async (long id, string action, AppDbContext db, ITenantContext t) =>
{
    var h = await db.RedeemInvoiceRequests.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.Id == id);
    if (h is null) return Results.NotFound(new { id });
...
app.MapPost("/api/salesmancerts", async (SalesManCertificateDto dto, AppDbContext db, ITenantContext t) =>
{
    ...
    if (dto.EffStartDate.HasValue && dto.EffEndDate.HasValue && dto.EffEndDate < dto.EffStartDate) return Results.BadRequest(new { error = "Ngày hết hiệu lực trước ngày bắt đầu." });
    ...
    row.CertificateName = dto.CertificateName; row.SMType = dto.SMType; row.DepartmentCode = dto.DepartmentCode; row.EffStartDate = dto.EffStartDate; row.EffEndDate = dto.EffEndDate; row.UpdatedAt = DateTime.Now;
    if (!string.IsNullOrWhiteSpace(dto.FlagActive)) row.FlagActive = dto.FlagActive!;
    await db.SaveChangesAsync();
...
{
    var row = await db.SalesManCertificates.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.Id == id);
    if (row is null) return Results.NotFound(new { id });
    row.FlagActive = row.FlagActive == "1" ? "0" : "1"; row.UpdatedAt = DateTime.Now;
    await db.SaveChangesAsync();
    return Results.Ok(new { row.Id, row.FlagActive });
...
app.MapPost("/api/stockoutorders/sv", async (SerStockOutOrderSvDto dto, AppDbContext db, ITenantContext t, HttpContext http) =>
{
    if (string.IsNullOrWhiteSpace((dto.RONo ?? "").Trim())) return Results.BadRequest(new { error = "Chưa nhập số lệnh sửa chữa (RO)." });
...
{
    var qry = db.DealerContractForms.Where(x => x.OrgId == t.OrgId);
    if (all != true) qry = qry.Where(x => x.FlagActive == "1");
...
app.MapPost("/api/dealercontractforms", async (DealerContractFormDto dto, AppDbContext db, ITenantContext t) =>
{
    var dealer = (dto.DealerCode ?? "").Trim();
    var fno = (dto.ContractFNo ?? "").Trim();
    if (string.IsNullOrWhiteSpace(dealer)) return Results.BadRequest(new { error = "Chưa nhập mã đại lý." });
    if (string.IsNullOrWhiteSpace(fno)) return Results.BadRequest(new { error = "Chưa chọn mã mẫu hợp đồng." });
    var row = await db.DealerContractForms.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.DealerCode == dealer);
    if (row is null) { row = new DealerContractForm { OrgId = t.OrgId, DealerCode = dealer }; db.DealerContractForms.Add(row); }
    row.ContractFNo = fno; row.ContractFName = dto.ContractFName; row.UpdatedAt = DateTime.Now;
    if (!string.IsNullOrWhiteSpace(dto.FlagActive)) row.FlagActive = dto.FlagActive!;
    await db.SaveChangesAsync();
    return Results.Ok(new { row.Id, row.DealerCode, row.ContractFNo, row.ContractFName, row.FlagActive });
}).RequireAuthorization();

app.MapPost("/api/dealercontractforms/{id}/toggle", async (long id, AppDbContext db, ITenantContext t) =>
{
    var row = await db.DealerContractForms.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.Id == id);
    if (row is null) return Results.NotFound(new { id });
    row.FlagActive = row.FlagActive == "1" ? "0" : "1"; row.UpdatedAt = DateTime.Now;
    await db.SaveChangesAsync();
    return Results.Ok(new { row.Id, row.FlagActive });
...
app.MapPost("/api/deliveryrequests", async (DeliveryRequestDto dto, AppDbContext db, ITenantContext t) =>
{
    var cars = dto.Cars ?? new();
    if (cars.Count == 0) return Results.BadRequest(new { error = "Cần ít nhất 1 xe." });
    var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
    foreach (var c in cars)
    {
        var cid = (c.CarId ?? "").Trim();
        if (cid == "") return Results.BadRequest(new { error = "Có dòng thiếu mã xe." });
        if (!seen.Add(cid)) return Results.BadRequest(new { error = "Trùng mã xe: " + cid });
    }
    var no = "DR" + DateTime.Now.ToString("yyMMddHHmmss");
...
app.MapPost("/api/deliveryrequests/{id}/action", async (long id, DRActionDto dto, AppDbContext db, ITenantContext t) =>
{
    ...
    (string[] from, string to) rule = act switch
    {
        "submit"  => (new[] { "Draft" }, "Sent"),
        "approve" => (new[] { "Sent" }, "Approved"),
        "reject"  => (new[] { "Sent" }, "Rejected"),
        _ => (Array.Empty<string>(), "")
    };
    if (rule.to == "") return Results.BadRequest(new { error = "Hành động không hợp lệ." });
    if (!rule.from.Contains(h.Status)) return Results.BadRequest(new { error = $"Không thể '{act}' khi đang {h.Status}." });
    if (act == "reject" && string.IsNullOrWhiteSpace(dto.Note)) return Results.BadRequest(new { error = "Từ chối phải ghi lý do." });
    h.Status = rule.to; if (!string.IsNullOrWhiteSpace(dto.Note)) h.Note = dto.Note; h.UpdatedAt = DateTime.Now;
    await db.SaveChangesAsync();
    return Results.Ok(new { h.Id, h.Status });
}).RequireAuthorization();
...
app.MapGet("/api/inventorycosts", async (AppDbContext db, ITenantContext t, string? storage, string? costType, string? active) =>
{
    var q = db.InventoryCosts.Where(x => x.OrgId == t.OrgId);
    if (!string.IsNullOrWhiteSpace(storage)) q = q.Where(x => x.StorageCode == storage);
...
// Upsert theo (kho + loại chi phí): mỗi cặp 1 đơn giá; nhập lại = cập nhật.
app.MapPost("/api/inventorycosts", async (InventoryCostDto dto, AppDbContext db, ITenantContext t) =>
{
    if (string.IsNullOrWhiteSpace(dto.StorageCode)) return Results.BadRequest(new { error = "Chưa chọn kho." });
    if (string.IsNullOrWhiteSpace(dto.CostTypeCode)) return Results.BadRequest(new { error = "Chưa chọn loại chi phí." });
    if (dto.UnitPrice < 0) return Results.BadRequest(new { error = "Đơn giá không hợp lệ." });
    var st = dto.StorageCode.Trim().ToUpperInvariant(); var ct = dto.CostTypeCode.Trim().ToUpperInvariant();
    var ex = await db.InventoryCosts.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.StorageCode == st && x.CostTypeCode == ct);
    if (ex is not null)
    {
        ex.StorageName = dto.StorageName; ex.CostTypeName = dto.CostTypeName; ex.UnitPrice = dto.UnitPrice; ex.FlagActive = "1";
        await db.SaveChangesAsync();
        return Results.Ok(new { ex.StorageCode, ex.CostTypeCode, updated = true });
    }
    var r = new InventoryCost { OrgId = t.OrgId, StorageCode = st, StorageName = dto.StorageName, CostTypeCode = ct, CostTypeName = dto.CostTypeName, UnitPrice = dto.UnitPrice, FlagActive = "1" };
    db.InventoryCosts.Add(r); await db.SaveChangesAsync();
    return Results.Ok(new { r.StorageCode, r.CostTypeCode, updated = false });
}).RequireAuthorization();

app.MapPost("/api/inventorycosts/{storage}/{costType}/toggle", async (string storage, string costType, AppDbContext db, ITenantContext t) =>
{
    storage = storage.Trim().ToUpperInvariant(); costType = costType.Trim().ToUpperInvariant();
    var x = await db.InventoryCosts.FirstOrDefaultAsync(v => v.OrgId == t.OrgId && v.StorageCode == storage && v.CostTypeCode == costType);
    if (x is null) return Results.NotFound(new { storage, costType });
    x.FlagActive = x.FlagActive == "1" ? "0" : "1";
    await db.SaveChangesAsync();
    return Results.Ok(new { x.StorageCode, x.CostTypeCode, flagActive = x.FlagActive });
}).RequireAuthorization();
...
{
    var query = db.SalesMen.Where(x => x.OrgId == t.OrgId);
    if (!string.IsNullOrWhiteSpace(q)) query = query.Where(x => x.SalesManCode.Contains(q!) || x.SalesManName.Contains(q!));
    if (!string.IsNullOrWhiteSpace(dealer)) query = query.Where(x => x.DealerCode == dealer);
    var rows = await query.OrderBy(x => x.SalesManCode).Take(500)
        .Select(x => new { x.Id, x.SalesManCode, x.SalesManName, x.DealerCode, x.DepartmentCode, x.SalesType }).ToListAsync();
    return Results.Ok(new { count = rows.Count, rows });
}).RequireAuthorization();

app.MapPost("/api/support/salesman-dept", async (List<SalesmanDeptFixDto> fixes, AppDbContext db, ITenantContext t) =>
{
    if (fixes is null || fixes.Count == 0) return Results.BadRequest(new { error = "Không có dòng nào." });
    int updated = 0; var changes = new List<object>();
    foreach (var f in fixes)
    {
        var s = await db.SalesMen.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.Id == f.Id);
        if (s is null) { changes.Add(new { f.Id, error = "Không tìm thấy NVBH." }); continue; }
        var dept = string.IsNullOrWhiteSpace(f.DepartmentCode) ? s.DepartmentCode : f.DepartmentCode.Trim();
        var type = string.IsNullOrWhiteSpace(f.SalesType) ? s.SalesType : f.SalesType.Trim();
        if (dept == s.DepartmentCode && type == s.SalesType) continue;
        changes.Add(new { s.Id, s.SalesManCode, oldDept = s.DepartmentCode, oldType = s.SalesType, newDept = dept, newType = type });
        s.DepartmentCode = dept; s.SalesType = type; updated++;
    }
    await db.SaveChangesAsync();
    return Results.Ok(new { total = fixes.Count, updated, changes });
}).RequireAuthorization();

// ===== Công cụ sửa ngày giao xe thực tế (Support — port 1:1 FrmSearchForCarDeliveryDate_Update/FrmSupportCarDeliveryDate_Update, 2010.HTC) =====
app.MapGet("/api/support/delivery-date", async (AppDbContext db, ITenantContext t, string? doNo, string? status) =>
{
    var query = db.DeliveryOrders.Where(x => x.OrgId == t.OrgId);
    if (!string.IsNullOrWhiteSpace(doNo)) query = query.Where(x => x.DoNo.Contains(doNo!));
    if (!string.IsNullOrWhiteSpace(status)) query = query.Where(x => x.Status == status);
    var rows = await query.OrderByDescending(x => x.Id).Take(500)
        .Select(x => new { x.Id, x.DoNo, x.Status, deliveredAt = x.DeliveredAt.HasValue ? x.DeliveredAt.Value.ToString("yyyy-MM-dd") : "" }).ToListAsync();
    return Results.Ok(new { count = rows.Count, rows });
}).RequireAuthorization();

app.MapPost("/api/support/delivery-date", async (List<DeliveryDateFixDto> fixes, AppDbContext db, ITenantContext t) =>
{
    if (fixes is null || fixes.Count == 0) return Results.BadRequest(new { error = "Không có dòng nào." });
    int updated = 0; var changes = new List<object>();
    foreach (var f in fixes)
    {
        if (!DateTime.TryParse(f.DeliveredAt, out var d)) { changes.Add(new { f.Id, error = "Ngày không hợp lệ." }); continue; }
        if (d.Date > DateTime.Today) { changes.Add(new { f.Id, error = "Ngày giao không được ở tương lai." }); continue; }
        var o = await db.DeliveryOrders.FirstOrDefaultAsync(x => x.OrgId == t.OrgId && x.Id == f.Id);
        if (o is null) { changes.Add(new { f.Id, error = "Không tìm thấy lệnh giao." }); continue; }
        changes.Add(new { o.Id, o.DoNo, oldDate = o.DeliveredAt.HasValue ? o.DeliveredAt.Value.ToString("yyyy-MM-dd") : "", newDate = d.Date.ToString("yyyy-MM-dd") });
        o.DeliveredAt = d.Date; updated++;
    }
    await db.SaveChangesAsync();
    return Results.Ok(new { total = fixes.Count, updated, changes });
}).RequireAuthorization();
